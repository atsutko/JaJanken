...
public extension KolodaViewDelegate {
    ...
    func koloda(_ koloda: KolodaView, allowedDirectionsForIndex index: Int) -> [SwipeResultDirection] { return [.left, .right, .up, .down] }
    func koloda(_ koloda: KolodaView, didSwipeCardAt index: Int, in direction: SwipeResultDirection) {}
    func kolodaDidRunOutOfCards(_ koloda: KolodaView) {}
    func koloda(_ koloda: KolodaView, didSelectCardAt index: Int) {}
...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    func card(cardAllowedDirections card: DraggableCardView) -> [SwipeResultDirection] {
        ...
        return delegate?.koloda(self, allowedDirectionsForIndex: index) ?? [.left, .right, .up, .down]
    }
